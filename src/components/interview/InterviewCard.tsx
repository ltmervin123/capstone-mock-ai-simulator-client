...
export default function InterviewCard({ type, title, description, handleCardSelect }: Props) {
  const currentColors = CLASS_COLOR[type];

  return (
    <div
      className={`rounded-lg border-2 ${currentColors.border} bg-white p-6 shadow-lg ${currentColors.shadow} transition-shadow ${currentColors.hoverShadow} flex flex-col justify-between backdrop-blur-sm`}
    >
      <div className="mb-6 grid place-items-center">
        <div
          className={`flex h-32 w-32 items-center justify-center rounded-full ${currentColors.bg} text-lg font-semibold text-white`}
        >
          {type}
        </div>
